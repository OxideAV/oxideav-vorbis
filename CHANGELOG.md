...
- a genuine **low-bitrate mode** below the quality knee
  ...
  ladder rungs (coarse step × 2 / × 4) were measured and rejected.
...
- vorbis README: r453 encoder status + measured gaps rewrite
- ABR bit targeting over the whole-stream encoder
- loudness-adaptive perceptual attack detector
- per-packet masking-driven coupling with shared pair floors
- masking-weighted VQ selection per read
- phase-predictability tonality in the masking model
- band-level psy recalibration, covering floor fit, full coded band, quantile ladder span, half-span + wide-cascade tiers
- per-channel block-size scheduling replaces the channel mixdown
- measured per-channel adaptive-margin balance pass at the top of the knob
- vorbis README: document the hostile-codebook allocation fence
- pin the endpoint-only (partitions=0) floor-1 decode corner
- fence hostile-codebook speculative allocations to the bit budget
- bootstrap the fleet-standard fuzz harness
- vorbis README: FFT-decomposed transforms + r447 differential campaign
- equal-blocksize long-mode decode coverage from the differential campaign
- FFT-decomposed forward MDCT sharing the DCT-IV core
- FFT-decomposed SS4.3.7 IMDCT — decode moves to O(N log N)
- vorbis README: r438 wiring + floor-0 fixture status
- end-to-end seek validation through demux + registry
- prose-hygiene scrub of implementation attributions in src comments
- closed-form structural conformance for the floor-0 fixture
- floor-0 real-stream fixture decodes sample-exact
- end-to-end container -> registry -> PCM wiring validation
- decode from Xiph-laced codec-private extradata
- crate description reflects the shipped codec, not the rebuild scaffold
- deep 8-D band tiers, measured band adoption, final-emission value-book retrain
- exact branch-and-bound VQ search over sparse full-grid lattices
- declare the SS4.2.1 payload magic on the codec registration
